...
| `breaker.py` | The circuit breaker in front of both Redis connections. `CircuitBreaker` is closed, open, or half-open with exactly one probe, on an injected monotonic clock. `BreakerOpen` subclasses `redis.RedisError`, so every existing failure policy keeps its meaning when a call is refused. `cost_breaker()` and `operational_breaker()` are built from `Settings` on first use. It logs `breaker_opened` / `breaker_closed` on transitions only. The provider half is not here. |
...
| Event | Level | When |
| --- | --- | --- |
| `breaker_opened` | `WARNING` | A connection's breaker opened, either after `DODEAL_BREAKER_FAILURE_THRESHOLD` consecutive failures or when its single half-open probe failed. Carries `breaker` (`cost` or `operational`). Calls refused while it stays open log nothing more here. |
