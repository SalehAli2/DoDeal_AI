...
- **Coached languages.** `unit_b.coaching_languages` (default the four Arabic codes and `en`): a call whose client language is not listed gets no coaching and no score, both `null` with `language_not_enabled`. Add a language only once it has been tested.
