...
- **Speakers and signals.** With the roles not applied the speaker is `unknown`: a number matching neither hash is `unattributed_number` and raises `off_channel_contact_review`; the lead's and the company's raise nothing. The talk signals are then `null` with `talk_reason` `roles_not_applied`. Otherwise `talk_balance` is `client_led` (agent share under 0.35), `balanced` or `agent_heavy` (over 0.65). `single_voice`, `long_call_no_diarization` and `no_speech` make a transcript uncertain. Details in `docs/contracts/call_events.md`.
- **Languages.** The roles pass also names each side's language (`call_languages`: `gulf_ar`, `egyptian_ar`, `levantine_ar`, `iraqi_ar`, `maghrebi_ar`, `msa_ar`, `en`, `hi`, `ur`, `ru`, `zh`, `fr`, `fa`, `tr` or `other`), each quoted from that side's own voice. The language profile and the summary language read them; the script is only the fallback when the pass did not run, failed or was not applied. Stage 1 shows which under `languages.source`.
- **WhatsApp suggestion.** Written in the client's language. To an Arabic speaker it is in the agent's dialect when the extras pass heard it (`agent_dialect`), else in `unit_b.whatsapp_default_dialect` (`gulf_ar`, `egyptian_ar`, `levantine_ar` or `iraqi_ar`; default `gulf_ar`). `POST /api/v1/calls/jobs/{job_id}/whatsapp {"language": ...}` writes it again in any of the 12 languages: one paid pass per language, held while the stage-1 result is, `409 result_expired` after.
- **Coached languages.** `unit_b.coaching_languages` (default every Arabic code, `gulf_ar`, `egyptian_ar`, `levantine_ar`, `iraqi_ar`, `maghrebi_ar` and `msa_ar`, plus `en`): a call is coached and scored only when **both** the agent's and the client's languages are listed; otherwise both are `null` with `language_not_enabled`. A side the roles pass did not hear counts as not listed; with neither heard, the transcript's script decides. Add a language only once it has been tested.
- **Engaged client.** A call is scored only when the client engaged: a talk share of at least `unit_b.engaged_share` (default 0.15), or at least `unit_b.engaged_turns` (default 5) turns of four words or more. Otherwise the score is `null` with `not_engaged`.
- **Time zone.** `unit_b.timezone` (an IANA name, default `Asia/Dubai`) is the company's zone: the extraction resolves a next step said on a call ("tomorrow at 5") from the call's `recorded_at` in that zone, and stage 1 delivers `next_step.when` with its offset. A wrong zone shifts every booked time; an unknown name is refused at the `PUT`.
- **When a segment was said (provisional, `ASSUMPTION[Q36]`).** A call's `recorded_at` is taken as when the call started, so a segment was said at `recorded_at` plus its `start_s`: code writes that moment on each extraction line, and a next step's time is held to it (from 60 s before to 365 days after). If the CRM sends the call's end, every stamp is late by the call's length; the fix is `CallClock.said_at` alone. The correction path is in `ASSUMPTIONS.md` §4.10.
- **call.translation retries.** Delivered on the callback schedule (60, 300, 1800, 7200 s), one delivery state per target; a retry resends the held result and never calls the model. The pass asks an unanswered chunk once more; the task is never re-run.
...
| `logging_config.py` | Configures Python logging once at application startup. Emits one structured JSON line per log record to standard output. Sets the `dodeal_ai` logger tree to the configured level so audit `allow` lines are not silently dropped, while leaving third-party libraries at the root logger's default level. `JsonFormatter` never formats an exception: a record logged with `exc_info` — by starlette's `ServerErrorMiddleware`, by uvicorn, by any library logging through our root handler — yields `exc_type` (module-qualified class) and `exc_frames` (the traceback frames via `log_safety.frames_only`), and the `exc_info` key is **gone**, because `formatException` printed `str(exc)` and every message in the `__cause__`/`__context__` chain (register item 88). `record.exc_text` is never read, for the same reason. |
| `log_safety.py` | The two helpers every exception-logging site uses, and — since register item 88 put `frames_only` inside `JsonFormatter` — the vocabulary EVERY log record in the process is held to, whoever wrote the log call. `safe_error_fields` returns the exception's type and module always, its message only when the class is defined under `dodeal_ai` (where messages are fixed vocabulary), and a chained exception's class name but never its message. `frames_only` returns the traceback frames without the exception line and without walking the chain. A foreign exception's message is frequently the data that failed, so it is never logged. |
...
| `test_log_exc_info.py` | The stdout-wide half of the same claim (register item 88): a foreign `RuntimeError` whose message is a sentinel, chained from a second sentinel, raised inside a test-only route on the real app and logged with `exc_info` the way a library does. Captured with `capfd` (the bytes on the file descriptor, so a uvicorn-style logger is seen too) under the real `configure_logging()`: neither sentinel reaches stdout or the 500 body, while `exc_frames` still names the failing route function. Also the same exception through `uvicorn.error` directly, the `exc_info` key's absence, and the `(type, None, None)`, all-`None` and bare-`True` record shapes. |
