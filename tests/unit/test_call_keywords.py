...
async def test_the_extras_result_uses_the_listed_canonical_name() -> None:
    """The list reaches the prompt; a listed name is kept, an unlisted refused."""
    answer = _answer()
    ...
    keywords = extras_part(_call(), found)["keywords"]
    ...
    with pytest.raises(OutputValidationError) as refused:
        check_extras(_call(), LISTED)(Extras.model_validate(answer))
    assert refused.value.errors == (("keywords.0.canonical", "not_listed"),)
