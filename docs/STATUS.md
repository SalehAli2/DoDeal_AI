...
| Step 3 — token counters. **N.1 (`dffeb80`)**: Redis timeouts, bounded pools, `/ready` reports db2, the fakeredis lane. **N.2 (`77df41d`)**: `enforce_token_cost`, the real fail-open `token_preflight` (the no-op stub and its grep marker are gone), `TokenBudgetExceeded` 429, the warning ratio. **N.4 (sha pending)**: the real-Redis lane. **Still owed:** the M4 TTL fix, policy-per-caller for fail-closed workers, and degradation behaviour at the warning ratio (A9). The H3 breaker, listed here as owed until now, landed in N.3 (`3199e3d`). | PARTIAL | `dffeb80` · `77df41d` |
