...
**Last updated:** 11 Sep 2026, after Unit A Project 1 phases A–J, Piece K, Piece L, Piece M and Piece N (N.1, N.2, N.3, N.3b) — the judgement pipeline is built end to end against `FakeLLM` and the vendored fake CRM (head of `scaffold/core-governance-homes`, CI green). **Nothing in Unit A has been verified against a real model or a real backend; nothing is marked `[V]`** (ASSUMPTIONS §8.11).
...
Suite at head: **1070 tests passing, 1 skipped, 7 deselected; 99.47% coverage** (total floor 92, plus 15
...
**Item 3 is still partial.** N.3 closes the H3 breaker's Redis half. Still owed: the M4 TTL fix on the two cost scripts (deliberately untouched here), and policy-per-caller for fail-closed workers — N.4.
