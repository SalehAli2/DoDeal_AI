...
**Last updated:** 16 Sep 2026, after Unit A Project 1 phases A–J, Piece K, Piece L, Piece M, Piece N (N.1, N.2, N.3, N.3b, N.4), Piece 103, Piece N.4b, Piece 76.1, Piece 102, Piece 95, Piece 85, Piece 86, Piece 87, Piece 88 and Batch F1 (items 125, 123, 117) — the judgement pipeline is built end to end against `FakeLLM` and the vendored fake CRM, and the LLM seam now has a real Groq/OpenAI adapter that has never been called (head of `scaffold/core-governance-homes`, CI green). **Nothing in Unit A has been verified against a real model or a real backend; nothing is marked `[V]`** (ASSUMPTIONS §8.11). Redis is the one dependency that has met a real server: the `redis_real` lane (N.4), by hand, against Redis 7.4.10.
...
| **Batch F1, the rule** — `CLAUDE.md`'s "Commits" rule, in the lead's words. An AI piece carries one register item per session. A foundation piece (Redis, middleware, logging, config, CI, test infrastructure, deployment, docs) may carry up to three, each in its own code commit, with one backfill closing the session. A piece unclear about its kind is treated as AI and stops after one item. This is what let items 125, 123 and 117 share one session. No register item. | DONE | `c2d4941` |
| **Batch F1, item 123** — the elapsed test's fake clock holds whole milliseconds from zero (register item 123, closed). `_FakeClock` in `tests/unit/test_judgement_pipeline.py` held a float, so at any realistic base the 20 ms the fetch advances reads 19 through `_ms_since`'s truncation. It now holds an integer `_ms` from 0, with no base to set, and `advance` takes whole milliseconds. **The cause is the base, not the float:** whole milliseconds alone still read 19 from a base of 1,000,000, so starting at zero is the fix, and the code comment says so. `__getattr__` now raises `AttributeError` instead of delegating to the real `time` module, so a second clock reading added to the pipeline fails the test instead of silently using the real clock. One new test. Both assertions of the elapsed test are unchanged. The module ran 20 times in a row, green each time. Test module only, no `src/` change. | DONE | `3edbd57` |
| **Batch F1, item 125** — a template cache miss warns once the cache is populated (register item 125, closed). `_load_template` in `core/prompting.py` now logs one WARNING `prompt_template_not_preloaded` per name (logger `dodeal_ai.prompting`, field `template`) when a name misses while `_TEMPLATE_CACHE` is non-empty. A miss there means a running app is sending a template the preload never named: a disk read on the event loop, and a missing file found on a paid call instead of at startup. It then reads from disk exactly as before. An empty cache (a script, the wheel check, a tmp-dir test) logs nothing. `_WARNED_NOT_PRELOADED` is cleared by `clear_templates()` along with the cache, so the next app in the process warns again. The line carries the name only, never the resolved path. A test parses every string constant under `src/dodeal_ai/` for `structured_intelligence/<name>_v<n>.txt`, finds exactly nine, and asserts each is in `UNIT_A_TEMPLATES`. Four tests. `core/prompting.py` is at 100 %, with no floor added (item 113 says none). | DONE | `1935d01` |
| **Batch F1, item 117** — a CRM note id is assumed unique and never reused (register item 117, closed). New `ASSUMPTIONS.md` §4.8: `[D]` at best, because no document states it and nobody has observed it. Three mechanisms rest on it: the per-note attempt cap (`attempt:{tenant}:{lead_id}:{note_id}`), the resubmission's `attempt_fp:` reference (item 33), and the six-hour `attempt_ttl_seconds` both keys carry. If an id is reused, the new note inherits the spent allowance and answers `attempt_cap`, and nothing in the logs tells the two notes apart. This was seen on the fake CRM, whose ids reset on restart. The section also gives the correction path and the seams. The question is Q22 in §6, owner Backend, **UNASKED**. Docs only. | DONE | `741b17a` |
...
Suite at head: **1293 tests passing, 1 skipped, 30 deselected; 99.59% coverage; 15 floors met**, identical with the compose Redis running or stopped (Piece 103). Piece 76.1 adds 115 tests (`tests/unit/test_openai_compatible_adapter.py`) and takes `core/llm/openai_compatible.py` to 100%. Total floor 92, plus 15
...
the deselected 30 are the `integration` marker (9) and the `redis_real` lane (21). Piece 86 adds 16 tests (`tests/test_no_base_http_middleware.py`, `tests/unit/test_middleware_asgi.py`) and leaves both middleware modules at 100%. Piece 87 adds 25 (`tests/unit/test_body_limit.py`) and takes `middleware/body_limit.py` to 100% as well, with no floor added for it. Piece 88 adds 7 (`tests/security/test_log_exc_info.py`) and takes `core/logging_config.py` to **100%**, with no floor added for it: the `exc_info` branch used to be reached only when something outside the tree happened to log one, which is the flip that moved the total between 99.51% and 99.57%. Three full runs at `77a2bbd` all read **1288 passed, 1 skipped, 30 deselected, 99.59% (99.5885%)**, with `core/logging_config.py` at 100% and no missing lines in any of them — the flip is gone. Batch F1 adds 5 tests: 1 in `tests/unit/test_judgement_pipeline.py` (item 123) and 4 in `tests/unit/test_prompt_preload.py` (item 125). It moves the total from 99.5885% to **99.5900%**, with `core/prompting.py` at 100% (56 statements) and no floor added for it, and all 15 floors met at every commit. The lane's own run, by hand against
...
### Register items closed in Batch F1

| # | Item | What landed | Commit |
| --- | --- | --- | --- |
| 123 | The elapsed test's fake clock holds whole milliseconds and refuses other attributes | **Closed.** An integer `_ms` from zero, `advance(20)`, and an `__getattr__` that raises. The cause was the base, not the float. Sabotage: restoring the delegating `__getattr__` fails the new test with `DID NOT RAISE`. | `3edbd57` |
| 125 | One WARNING `prompt_template_not_preloaded` per name on a miss against a populated cache; a `src/` scan against `UNIT_A_TEMPLATES` | **Closed.** Logger `dodeal_ai.prompting`, field `template`, name only, once per name per app. Sabotage: removing the log call fails 2 tests; dropping `SCORE_TEMPLATE` from the tuple fails 4, not the 2 predicted. | `1935d01` |
| 117 | `ASSUMPTIONS.md` records that a CRM note id is unique and never reused | **Closed** as a record: `ASSUMPTIONS.md` §4.8 and question Q22 in §6. The question itself is still UNASKED and owed to the backend. | `741b17a` |
