...
**State at this checkpoint (branch `call-intelligence`, Unit B batch 2: stuck jobs and wave 1).** No real speech-to-text adapter exists yet. `build_transcriber` refuses to build one, and the only transcriber is `FakeTranscriber`. **Call content now reaches a language model**: wave 1 runs two passes, `unit_b.extract` and `unit_b.prose` (`units/call_intelligence/passes.py`), on every transcribed call, and stage 1 carries their analysis. Signals, number detection and alarm phrases run in code with no model. Wave 2 (objections, scoring, coaching, tags, WhatsApp, seriousness), voice ID and prosody are not built. Where a control belongs to a pass that has not been built, the paragraph says so and names the batch that owes it. "Not built" means exactly that; it is not a claim that the risk is absent.
