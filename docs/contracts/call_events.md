  ...
  "result": {
    ...
    "analysis": {
      ...
      "elements": {
        ...
        "discussed": [{"text": "budget", "quote": "my budget is 1,200,000 AED", "segment": "s2", "unverified": false},
                      {"text": "a meeting", "quote": "Shall we meet on Tuesday", "segment": "s4", "unverified": false}],
        "concerns": [],
        ...
        "next_step": {"action": "Meet", "owner": "agent", "due": "Tuesday",
                      ...
                      "kind": "office_visit", "when": null, "when_quote": null, "when_segment": null,
                      "when_state": "uncertain", "when_reason": null, "booked": false},
...
| `analysis.elements` | **The evidence fields.** `wanted` (or `null`), every `discussed[]`, `concerns[]` and `agreed[]` item (each `{text, quote, segment, unverified}`), the `next_step` and the `loss_reason` carry a `quote` and the `segment` it is from, checked in code; the same holds for `details` and `mood`. Each quote is asked for at most 15 words, and the check in code allows up to 40. Its words, normalised (case, Arabic letter forms and marks), must appear in the segment it cites in the same order. Between two of them the segment may hold only a word that repeats the word before it ("عمري عمري") or a filler of the versioned list `quote_fillers_v1` (يعني، اه، ايوه، um, uh and the like); a negation (ما، مش، لا، لم، لن، مو، ليس, not, no, never, don't and the like) is never a filler. A quote not in the segment it cites is looked for in the segment just before, then just after, and is delivered with the `segment` it was found in. An element whose quote fails is kept with `unverified: true` and its `quote` and `segment` `null`; the summary and CRM note are written from the verified elements only, so an unverified one never reaches them. `next_step.owner` is `agent`, `client` or `unknown`; its `action`, `due`, `quote`, `segment`, `kind` and `when` are `null` and `booked` is `false` when there is none. `ending` is `moved_forward`, `stalled`, `needs_follow_up` or `dead`. |
| `analysis.elements.next_step` | `kind`: `viewing`, `online_meeting`, `office_visit`, `callback`, `send_details` or `other`; present exactly when there is an `action`. **The time (`unit_b_prompts_v18`).** Each transcript line the model reads carries when it was said, written in code: `recorded_at` plus the segment's `start_s`, in the tenant's `timezone` (`unit_b.timezone`, default `Asia/Dubai`), as `YYYY-MM-DD HH:MM Weekday`. The model resolves the time against the line that names it ("بعد 3 دقايق" said at 05:00 on a call recorded at 10:00 is 10:08; "بكرة الساعة 5" is the next day at 17:00) and quotes those words. `when_quote` and `when_segment`: the words that name the time and the segment they were found in, under the quote rule; `null` when no time was given, and removed when the quote fails. `when`: ISO 8601 with the zone's offset, to the minute (seconds always `00`). Code keeps it only from 60 seconds before to 365 days after the moment the `when_quote`'s segment was said; else `when` is `null`. `when_reason`: why code dropped a time the model gave, else `null`: `when_missing_quote` (no verified `when_quote`), `when_out_of_range` (outside that window) or `when_no_anchor` (the call's `recorded_at` is unknown, so there is no moment to hold the time to). `when_state`: `stated` for a kept time on a verified next step; `uncertain` when a time was said but was vague ("next week sometime"), dropped by code, or rests on an unverified quote, and `when` is then `null`; `not_mentioned` when no time was said. `booked` (D-77, `unit_b_prompts_v19`): the next step's `quote` is then the agreement, the other speaker's short assent ("اوكي", "تمام", "ماشي", "ok"), which may be one word. `booked` is `true` only when the model said so and gave a time, the `when_quote` and the agreement quote are both found by the quote check, the segments they are found in are two different voices (neither `unknown`), and those segments are at most 3 apart, before or after. A `when_missing_quote` therefore leaves it `false`; `when_out_of_range` and `when_no_anchor` leave it as that rule gives it. **`ASSUMPTION[Q36]`:** `recorded_at` is when the call started; if the CRM sends its end, every said-at moment is late by the call's length. |
...
- **`roles.call_languages`.** The roles pass also answers `{client, agent}`, each `{language, quote, segment}`. `language` is one of `gulf_ar`, `egyptian_ar`, `levantine_ar`, `iraqi_ar`, `maghrebi_ar`, `msa_ar`, `en`, `hi`, `ur`, `ru`, `zh`, `fr`, `fa`, `tr` or `other`, judged from the words and not the script: Urdu is `ur`, never Arabic, and French is `fr`, never English. The quote is checked, in code, to come from a voice the answer gives that side's role. A side not heard is all `null`. Each side's language is judged from the call's opening and each voice's eight longest segments from anywhere in the call, so a dialect that shows only late is heard; the quote still comes from that side's own segment, under its own id.
...
| `score` | call_rubric_v2. Every `mark`, the `raw` sum, the `applicable_weight`, the `total` (0 to 100) and the `band` are computed in code, never by a model. `band` is `excellent` (85 and up), `good` (70 to 84), `needs_work` (50 to 69) or `coaching_required` (under 50). A component with `mark: null` is `suppressed`: `no_objections`, or `no_product_question`. It is left out of `applicable_weight`, not scored zero. A marked product_knowledge carries `correctness_unverified: true`. A check with `source: code` was decided in code, not by the model. **The escalations agree (D-75):** a verified `over_promise_or_guarantee` escalation the agent said sets `professionalism.checks.no_over_promise` to `{answer: "no", source: "code", quote, segment}`, the escalation's words, whatever the score pass answered, and code marks professionalism, the `total` and the `band` again. Nothing changes for one the client or an unknown voice said, one unverified, or when the escalations part is `null`. This is the only mapping; `rudeness_or_pressure` fails no check, since it may be rudeness alone or pressure alone. |
...
| `extras` | `keywords[]`: `kind` (`project`, `community`, `developer` or `topic`), `said` as spoken, a name of at most five words (a longer one is dropped), checked in its `segment`, and `english` or `null`. `tags`: `outcome` (`moved_forward`, `stalled`, `needs_follow_up`, `dead`), `stage` (`first_contact`, `follow_up`, `viewing`, `negotiation`, `closing`) and `client_type` (`end_user`, `investor`, `broker`, `unknown`). `whatsapp_suggestion`: at most 60 words in `language`; a text over 60 words or in the wrong script is asked for once more, and failing again is `null` with `whatsapp_reason` (`too_long` or `wrong_language`), the rest of the extras delivered. **This service never sends it**; show it to the agent to send or not. `seriousness`: five checks, each with a `reason` and a quote for a yes. The `band` is computed in code from the `yes` count, verified yes answers only: `A` for 4 or 5, `B` for 2 or 3, `C` for 0 or 1. **Field by field**: a keyword whose quote fails is dropped; a check whose quote fails is kept with `unverified: true` and its `quote` and `segment` `null`, and an unverified yes is not counted; the tags and the WhatsApp suggestion are always kept. **`manager_only: true`**: show it to the agent's manager, never to the agent. |
...
| `extras.keywords[].canonical` | the company's `keyword_vocabulary` name this keyword is, copied exactly as listed; `null` when it is none, or when the model named one not on the list, and the keyword is kept as found |
